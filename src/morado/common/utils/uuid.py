...
def generate_uuid(config: Optional[UUIDConfig] = None) -> str:
    """Generate UUID based on configuration"""
...
def generate_custom_uuid(
    ...
    length: Optional[int] = None,
    charset: str = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789",
    random_part_length: Optional[int] = None,
    use_timestamp: bool = False,
    secure: bool = False
) -> str:
...
def generate_distributed_uuid(
    ...
    actual_length = length if random_part_length is None else (len(full_prefix) + len(suffix) + random_part_length)
