...
class TimeUtil:
    ...
    def format_time(dt: datetime, format_string: str) -> str:
        ...
        if not isinstance(format_string, str):
            raise TypeError(f"Expected string format, got {type(format_string).__name__}")
    ...
    def parse_iso8601(time_string: str) -> datetime:
        ...
        if not isinstance(time_string, str):
            raise TypeError(f"Expected string, got {type(time_string).__name__}")
        ...
        try:
            ...
            raise TimeParseError(
                time_string,
                message=f"Failed to parse ISO 8601 time string '{time_string}': {str(e)}"
            )
    ...
    def parse_time(time_string: str, format_string: str) -> datetime:
        ...
        """
        if not isinstance(time_string, str):
            raise TypeError(f"Expected string for time_string, got {type(time_string).__name__}")
        
        if not isinstance(format_string, str):
            raise TypeError(f"Expected string for format_string, got {type(format_string).__name__}")
        ...
        try:
            ...
            raise TimeParseError(
                time_string,
                format_string,
                message=f"Failed to parse time string '{time_string}' with format '{format_string}': {str(e)}"
            )
    ...
    def time_difference(dt1: datetime, dt2: datetime) -> timedelta:
        ...
        if not isinstance(dt1, datetime):
            raise TypeError(f"Expected datetime for dt1, got {type(dt1).__name__}")
        
        if not isinstance(dt2, datetime):
            raise TypeError(f"Expected datetime for dt2, got {type(dt2).__name__}")
    ...
    def add_duration(dt: datetime, **kwargs) -> datetime:
        ...
        try:
            ...
            raise TypeError(f"Invalid duration arguments: {str(e)}")
    ...
    def add_to_time(dt: datetime | None = None, utc: bool = True, **kwargs) -> datetime:
        ...
        if dt is None:
            ...
            if not isinstance(dt, datetime):
                raise TypeError(f"Expected datetime object or None, got {type(dt).__name__}")
            base_time = dt
        
        return TimeUtil.add_duration(base_time, **kwargs)
    
    @staticmethod
    def subtract_from_time(dt: datetime | None = None, utc: bool = True, **kwargs) -> datetime:
    ...
    def convert_timezone(dt: datetime, target_tz: str | ZoneInfo) -> datetime:
        ...
        if isinstance(target_tz, str):
            try:
                ...
                raise ValueError(
                    f"Invalid timezone name '{target_tz}': {str(e)}. "
            ...
            raise TypeError(
                f"Expected string or ZoneInfo for target_tz, got {type(target_tz).__name__}"
            )
